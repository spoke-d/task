...
const (
	...
	grandParent parentType = "grand-parent"
	otherParent parentType = "other-parent"
)
...
func TestWithContext(t *testing.T) {
	...
	if child2.Value(parent) != grandParent {
	...
	parent3 := context.WithValue(context.Background(), parent, otherParent)
	child3 := tb.Context(parent3)

	if child3.Value(parent) != otherParent {
		t.Fatalf("Child context didn't inherit its parent's properties")
	}
	select {
	case <-child3.Done():
